...
import {
	SessionDatabase,
	type SimilarChunkRecord,
	type JsonlFileWithChunksRecord,
	type WorkspaceJsonlFileRecord,
...
interface GenericRerankApiItem {
	index?: unknown;
...
interface GenericRerankApiResponse {
	data?: unknown;
	results?: unknown;
}

interface RankedChunkResult extends SimilarChunkRecord {
	vectorScore: number;
	rerankScore: number;
	finalScore: number;
	mode: 'vector-only' | 'hybrid';
}
...
function buildJsonlFileKey(jsonlAbsolutePath: string): string {
...
function readStringSetting(
	...
	const raw = configuration.get<string>(key);
...
function readNumberSetting(
	...
	const raw = configuration.get<number>(key);
...
function resolveRuntimeConfiguration(): RuntimeConfiguration {
	...
	const configuredCohereApiKey = configuration.get<string>('cohereApiKey')?.trim() ?? '';
	...
	return {
		...
		embeddingDimensions: readNumberSetting(
			configuration,
			'embeddingDimensions',
		...
		maxTokensPerChunk: readNumberSetting(
			configuration,
			'maxTokensPerChunk',
			DEFAULT_MAX_TOKENS_PER_CHUNK,
			1,
		),
		chunkOverlapTokens: readNumberSetting(
			configuration,
			'chunkOverlapTokens',
...
function resolveCohereApiKey(configuredToken: string): string {
...
function findFirstValueByKey(value: unknown, key: string): unknown {
	if (Array.isArray(value)) {
...
function findPromptByPattern(value: unknown): unknown {
	if (!isRecord(value)) {
		return undefined;
	}
...
function findCustomTitleByPattern(value: unknown): string | undefined {
	if (!isRecord(value)) {
		return undefined;
	}
...
function timestampToDateTime(unixTimestamp: unknown): string {
...
function validateJsonlPath(inputPath: string): void {
...
function buildMarkdownContent(title: string, conversation: ConversationEntry[]): string {
	const lines: string[] = [];
	lines.push(`# ${title}`);
	lines.push('');
...
function resolveMarkdownTempDirectory(): string {
...
function buildTempMarkdownName(jsonlAbsolutePath: string, storageFolder: string): string {
...
async function convertJsonlToMarkdownInTemp(inputPath: string, storageFolder: string): Promise<string> {
	...
	for (let index = 0; index < rawLines.length; index += 1) {
		...
		let parsedLine: JsonValue;
		try {
			parsedLine = JSON.parse(trimmed) as JsonValue;
		} catch {
			continue;
		}
...
function splitLongSentenceIntoChunks(tokens: string[], maxTokens: number, overlap: number): string[] {
	const chunks: string[] = [];
...
function getOverlapText(sentences: string[], overlap: number): string {
...
function extractRerankScores(payload: unknown, candidateCount: number): number[] {
	if (!payload || typeof payload !== 'object') {
		throw new Error('La respuesta de reranking no es un JSON valido.');
	}

	const response = payload as GenericRerankApiResponse;
	...
	const scores = new Array<number>(candidateCount).fill(Number.NEGATIVE_INFINITY);
	...
	for (const rawItem of rawItems) {
		...
		const item = rawItem as GenericRerankApiItem;
...
function mapVectorResultsToFinal(results: SimilarChunkRecord[]): RankedChunkResult[] {
	return results
		.map((result) => ({
			...result,
...
async function rerankVectorSearchResults(
	queryText: string,
	candidates: SimilarChunkRecord[],
	cohereApiKey: string,
	cohereRerankModel: string,
	limit: number,
): Promise<RankedChunkResult[]> {
	...
	return candidates
		.map((candidate, index) => {
			...
			const finalScore = computeWeightedFinalScore(
				vectorScore,
				rerankScore,
				DEFAULT_VECTOR_SCORE_WEIGHT,
				DEFAULT_RERANK_SCORE_WEIGHT,
			);
...
function extractEmbeddingVector(payload: unknown, expectedDimensions: number): number[] {
	if (!payload || typeof payload !== 'object') {
		throw new Error('La respuesta de embeddings no es un JSON valido.');
	}

	const response = payload as ApiEmbeddingResponse;
	...
	if (Array.isArray(response.embedding)) {
		...
		const firstDatum = response.data[0] as ApiEmbeddingDatum;
	...
	if (vector.length !== expectedDimensions) {
		throw new Error(
			`Se esperaba un embedding de ${expectedDimensions} dimensiones y se recibio ${vector.length}.`,
		);
	}
...
async function requestEmbedding(input: string, runtimeConfiguration: RuntimeConfiguration): Promise<number[]> {
...
function summarizeChunk(text: string, maxLength = 140): string {
...
function logDetectedJsonlFiles(title: string, jsonlFiles: DetectedJsonlFile[]): void {
...
function logReindexedFiles(title: string, files: JsonlFileWithChunksRecord[]): void {
...
function logVectorSearchResultsInConsole(results: SimilarChunkRecord[]): void {
	...
	for (let index = 0; index < results.length; index += 1) {
		...
		console.log(
			`[historico] #${index + 1} score=${result.score.toFixed(6)} | chunk=${result.chunkIndex} | ${result.jsonlAbsolutePath} | ${summarizeChunk(result.chunkText)}`,
		);
	}
}

function logHybridRankingResultsInConsole(results: RankedChunkResult[]): void {
	...
	for (let index = 0; index < results.length; index += 1) {
		const result = results[index];
		console.log(
			`[historico] #${index + 1} final_score=${result.finalScore.toFixed(6)}, vector_score=${result.vectorScore.toFixed(6)}, rerank_score=${result.rerankScore.toFixed(6)}, mode=${result.mode} | chunk=${result.chunkIndex} | ${result.jsonlAbsolutePath}`,
		);
	}
}

async function collectChatSessionJsonlFiles(candidateFolder: string): Promise<DetectedJsonlFile[]> {
	...
	return detected.filter((item): item is DetectedJsonlFile => Boolean(item));
}
...
function buildModelContextFromDetectedPaths(detectedFiles: DetectedJsonlFile[]): string {
...
function buildVectorSearchContext(results: SimilarChunkRecord[]): string {
	if (results.length === 0) {
		return 'No hubo coincidencias en la busqueda vectorial para el prompt recibido.';
	}

	const lines = results.map((result, index) => (
		`- #${index + 1} (score=${result.score.toFixed(6)}): ${result.jsonlAbsolutePath} | chunk=${result.chunkIndex} | ${summarizeChunk(result.chunkText, 180)}`
	));

	return `Resultados de busqueda vectorial por chunks (${results.length}):\n${lines.join('\n')}`;
}

function buildHybridRankingContext(results: RankedChunkResult[]): string {
	if (results.length === 0) {
		return 'No hubo resultados finales tras aplicar reranking.';
	}

	const lines = results.map((result, index) => (
		`- #${index + 1} (final=${result.finalScore.toFixed(6)}, vector=${result.vectorScore.toFixed(6)}, rerank=${result.rerankScore.toFixed(6)}, mode=${result.mode}): ${result.jsonlAbsolutePath} | chunk=${result.chunkIndex}`
	));
...
function toJsonlWorkspaceRecord(currentWorkspacePath: string, file: DetectedJsonlFile, mdAbsolutePath: string): WorkspaceJsonlFileRecord {
	return {
...
function sumChunkCounts(records: JsonlFileWithChunksRecord[]): number {
	let total = 0;
...
export function activate(context: vscode.ExtensionContext): void {
	...
	const participant = vscode.chat.createChatParticipant(CHAT_PARTICIPANT_ID, async (request, _chatContext, response, token) => {
		...
		const detectedByFolder = await Promise.all(
			matchingFolders.map((folder) => collectChatSessionJsonlFiles(folder)),
		);
		...
		const filesByKey = new Map<string, DetectedJsonlFile>();
		...
		try {
			...
			const storedByKey = new Map<string, WorkspaceJsonlFileRecord>();
			for (const item of storedFiles) {
				storedByKey.set(buildJsonlFileKey(item.jsonlAbsolutePath), item);
			}

			await database.deleteFilesMissingFromWorkspace(
				currentWorkspacePath,
				deduplicatedFiles.map((file) => file.jsonlAbsolutePath),
			);

			const filesToReindex: DetectedJsonlFile[] = [];
			const alreadyIndexedFiles: WorkspaceJsonlFileRecord[] = [];
			...
			const reindexedFiles: JsonlFileWithChunksRecord[] = [];
			const failedFiles: Array<{ jsonlAbsolutePath: string; reason: string }> = [];
			...
			for (let fileIndex = 0; fileIndex < filesToReindex.length; fileIndex += 1) {
				...
				try {
					...
					const chunks = chunkText(
						markdownContent,
						runtimeConfiguration.maxTokensPerChunk,
						runtimeConfiguration.chunkOverlapTokens,
					);

					const chunkRecords: ChunkEmbeddingRecord[] = [];
					for (let chunkIndex = 0; chunkIndex < chunks.length; chunkIndex += 1) {
						response.progress(
							`Embedding chunk ${chunkIndex + 1}/${chunks.length} del fichero ${file.sessionFile}`,
						);
					...
					const recordToStore: JsonlFileWithChunksRecord = {
						...fileRecord,
						chunks: chunkRecords,
					};
			...
			let vectorSearchResults: SimilarChunkRecord[] = [];
			let hybridRankedResults: RankedChunkResult[] = [];
			if (promptForVectorSearch.length > 0) {
				...
				vectorSearchResults = await database.searchSimilarChunksByEmbedding(
					currentWorkspacePath,
					promptEmbedding,
					DEFAULT_VECTOR_RESULTS_LIMIT,
				);
				...
				if (vectorSearchResults.length > 0) {
					...
					try {
						...
						hybridRankedResults = await rerankVectorSearchResults(
							promptForVectorSearch,
							vectorSearchResults,
							cohereApiKey,
							runtimeConfiguration.cohereRerankModel,
							DEFAULT_RERANK_RESULTS_LIMIT,
						);
					} catch (error) {
						const message = error instanceof Error ? error.message : String(error);
			...
			processingOutcome = {
				statusMessage:
					`Se detectaron ${deduplicatedFiles.length} rutas JSONL absolutas. ` +
